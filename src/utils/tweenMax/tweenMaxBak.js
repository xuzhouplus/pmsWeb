...
class TweenMax {
	...
	switch(reverse, completed) {
		...
		this.switchImage(reverse, () => {
			this.switchCaption(reverse, completed)
	...
	switchCaption(reverse, completed) {
		if (!this.options.caption) {
			completed && completed()
			return
		}
		...
		let currentImage = this.getCurrentImage()
		currentImage.render.renderer.switchCaption(this.captionElement, reverse, () => {
