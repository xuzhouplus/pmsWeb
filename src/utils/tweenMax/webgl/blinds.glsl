vec2 blindsTransform(vec2 texCoord, float theta, vec2 axisPos, vec2 gridNum, float zOffset) {
...
vec4 blinds() {
    ...
    // 对纹理坐标进行变换，以及纹理采样
    vec2 texCoordAfterTransform = blindsTransform(vUv, rotateTheta, axisPos, gridNum, zOffset);
    texCoordAfterTransform.y = uv.y;
    vec4 texColor1 = texture(currentImage, texCoordAfterTransform);

    vec2 texCoordAfterTransform2 = blindsTransform(vUv, rotateTheta + pi / 3.0, axisPos, gridNum, zOffset);
